...
class ActionExtractor(AbsExtractor):
    ...
    def __init__(self, weights: (float, float, float) = (0.9, 0.095, 0.005), minimal_length_of_tokens: int = 3):
        ...
        self.cnlClient: CoreNLPClient = None

    def setClient(self, client: CoreNLPClient):
            ...
            for mention in corefs[cluster]:
                ...
                for pattern in self._evaluate_tree(trees[mention['sentNum'] - 1]):
                    ...
                    if re.sub(r'\s+', '', mention['text']) in np_string:
                        ...
                        for p in pattern[0]:
                            if isinstance(p[0], dict) and p[0]['nlpToken']['ner'] == 'MONEY':
                                candidate_object = Candidate()
                                candidate_object.set_sentence_index(pattern[2])
                                candidate_object.set_raw([pattern[0], pattern[1], cluster, mention['id']])
                                candidates.append(candidate_object)
    ...
    def _evaluate_candidates(self, document):
        ...
        # for c in document.get_candidates(self.get_id()):
        #    if isinstance(c, Candidate):
        #        raw = c.get_raw()
        #        for item in raw:
        #            if isinstance(item, list):
        #                ner = item[0][0]['nlpToken']['ner']
        #                if ner == 'MONEY':
        #                   money.append(item) --#
        #  if ner == 'MONEY':
        #    money.append(c)
        # for x in c[0]:
        #    ner = x[0]['nlpToken']['ner']
        #    if ner == 'MONEY':
        #        money.append(x[0])
        # print(money)
        ...
        # get Money by tokens regex
        money_pattern = '[{ner:"MONEY"}]+'
        matches = self.cnlClient.tokensregex(document.get_raw()['title'], money_pattern)
        ...
        for sentence in enumerate(sentences):
            for key in sentence[1]:
                if key != 'length':
                    ...
                    result.append(text)
        document.set_money_answer(result)
        # get Location by tokens regex
        location_pattern = '[{ner:"CITY"} | {ner:"STATE_OR_PROVINCE"} | {ner:"LOCATION"} | {ner:"COUNTRY"}]+'
        matches_location = self.cnlClient.tokensregex(document.get_raw()['title'], location_pattern)
        location_sentences = matches_location['sentences']
        result = []
        for sentence in enumerate(location_sentences):
            for key in sentence[1]:
                if key != 'length':
                    text = sentence[1][str(key)]['text']
                    result.append(text)
        document.set_location_answer(result)
